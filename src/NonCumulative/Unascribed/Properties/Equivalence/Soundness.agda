...
module NonCumulative.Unascribed.Properties.Equivalence.Soundness (fext : ∀ {ℓ₁ ℓ₂} → Extensionality ℓ₁ ℓ₂) where

open import Data.List using (drop)

open import Lib

open import NonCumulative.Ascribed.Statics.Presup as A
open import NonCumulative.Ascribed.Statics.CtxEquiv as A
open import NonCumulative.Ascribed.Statics.Refl as A
open import NonCumulative.Ascribed.Statics.Misc as A
open import NonCumulative.Ascribed.Statics.Inversion as A
open import NonCumulative.Ascribed.Statics.Properties.Contexts as A
open import NonCumulative.Ascribed.Statics.Properties as A
open import NonCumulative.Ascribed.Completeness.Consequences fext
open import NonCumulative.Ascribed.Consequences fext
...
⫢⊢∷  {Γ′} ⫢Γ′ ⫢T′
  ...
  where
    ...
    helper ℕ.zero (↝∷ Γᵢ↝ Tᵢ↝) (⊢∷ ⊢Γᵢ ⊢Tᵢ) 
      ...
      with refl , T≈Tᵢ ← IH-transform IHT Tᵢ↝ (ctxeq-tm (⊢≈-trans (⊢≈-sym Γᵢ≈Γ) Γ≈Γ₁) ⊢Tᵢ) ⊢T
...
⫢vlookup ⫢Γ′ x∈Γ′
  with Γ , Γ↝ , ⊢Γ , IHΓ ← ⫢Γ′
  with  _ , _ , T↝ , x∈Γ ← U⇒A-vlookup Γ↝ x∈Γ′ = -, -, -, -, Γ↝ , ↝v , T↝ , vlookup ⊢Γ x∈Γ , IHΓ , λ { ↝v ⊢v → ≈-refl ⊢v }
...
... | Δ , S , ⊢[|ze] , ⊢T , ≈Sei
  ...
  with refl , ≈N ← ze-inv ⊢ze = -, refl , ctxeq-tm (⊢≈-sym ≈Δ) ⊢T
...
... | Δ , S , ⊢[wkwk,suv1] , ⊢T , ≈Sei
  ...
  with refl , _ ← su-inv ⊢suv1 = -, refl , ctxeq-tm (⊢≈-sym ≈Δ) ⊢T
...
⫢s-I : ⫢ U.Γ′ →
       ...
       U.Γ′ ⫢s I ∶ U.Γ′
⫢s-I ⫢Γ′
  with Γ , Γ↝ , ⊢Γ , IHΓ ← ⫢Γ′ = -, -, -, Γ↝ , Γ↝ , ↝I , s-I ⊢Γ , IHΓ , (λ { ↝I ⊢σᵢ → s-≈-refl ⊢σᵢ }) , IHΓ
...
⫢s-wk ⫢T∷Γ′
  ...
  = -, -, -, ↝∷ Γ↝ T↝ , Γ↝ , ↝wk , s-wk (⊢∷ ⊢Γ ⊢T) , IHΓ , (λ { ↝wk ⊢σᵢ → s-≈-refl ⊢σᵢ }) , IHCΓ
...
qqσ-inv ⊢qqσ 
  ...
  with _ , ⊢∷ ⊢Ψ ⊢S₂ ← presup-⊢≈ Ψ≈
  with θ , θ≈ , ⊢σ ← qσ-inv ⊢qσ = -, ⊢σ , ⊢≈-trans Ψ≈ (∷-cong-simp θ≈ (≈-refl ⊢S₂))
